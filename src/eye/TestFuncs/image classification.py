...
from image_classification_functions import config
...
def train_model(cfg: config):
    ...
    for x in range(cfg.num_CNN_blocks):
        i=x+1
        model = cfg.add_CNN_block(model, cfg, x_train.shape[1:], i)
        
    # flatten the data and add another dropout layer
    model.add(keras.layers.Flatten())
    model.add(keras.layers.Dropout(cfg.dropout_fraction))

    # add some dense layers to the model
    for y in range(cfg.num_dense_layers):
        j=y+1
        model = cfg.add_dense_layer(model,cfg, j)

    # tell the model how many categories to classify the data into
    # dense_activation_type = softmax = highest probability voting.
    model.add(keras.layers.Dense(class_num, activation=cfg.dense_activation_type))
    ...
    history = model.fit(x_train, y_train, validation_data=(x_test, y_test), epochs=cfg.numEpochs, batch_size=cfg.batchSize)
